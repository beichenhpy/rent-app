		...
		<view class="content">
			...
			<view class="main">
				<view class="main-list oBorder">
					<input class="main-input" v-model="registerData.username" type="text" maxlength="11" placeholder="手机号" @blur="checkUser()" />
				</view>
				<wInput v-model="registerData.password" type="password" maxlength="11" placeholder="登录密码" isShowPass></wInput>
				<wInput v-model="registerData.verCode" type="number" maxlength="6" placeholder="验证码" isShowCode ref="runCode" @setCode="getVerCode()"></wInput>
			</view>

			<wButton text="注 册" :rotate="isRotate" :isDisable="isRegister" @click.native="startReg()"></wButton>
...
export default {
	...
	methods: {
		...
		checkUser() {
			if (!(/^1[3456789]\d{9}$/.test(_this.registerData.username))) {
				uni.showToast({
					icon: 'none',
					position: 'bottom',
					title: '手机号不正确'
			...
			if (_this.registerData.username !== '' || _this.registerData.username.length === 11) {
				uni.request({
					url: http.BaseUrl + 'user/checkUser/' + _this.registerData.username,
					method: 'POST',
		...
		getVerCode() {
			...
			uni.request({
				url: http.BaseUrl + 'user/sendSms/' + _this.registerData.username,
				method: 'POST',
				success: function(res) {
					if (res.data.code === 200) {
		...
		startReg() {
			...
			if (this.registerData.verCode.length != 6) {
				uni.showToast({
					icon: 'none',
					position: 'bottom',
					title: '验证码格式不正确'
