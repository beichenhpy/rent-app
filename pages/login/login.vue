<template>
	<view class="login">
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="login.username" type="text" maxlength="11" placeholder="手机号"></wInput>
				<wInput v-model="login.password" type="password" maxlength="11" placeholder="密码"></wInput>
			</view>
			<wButton text="登 录" :rotate="isRotate" @click.native="startLogin()" class="wbutton"></wButton>
			<wButton text="取 消" :rotate="isRotate" @click.native="cancle()" class="wbutton"></wButton>
			<!-- 底部信息 -->
			<view class="footer">
...
export default {
	...
	methods: {
		...
		startLogin() {
			...
			if (!(/^1[3456789]\d{9}$/.test(_this.login.username))) {
				uni.showToast({
					icon: 'none',
					position: 'bottom',
					title: '手机号格式不正确'
			...
			uni.request({
				url: http.BaseUrl + 'uaa/oauth/login',
				data: _this.login,
				method: 'POST',
